...
*/
namespace ZozoEMA\TopBar\Admin;
...
class Manager {
    ...
    {
		$this->plugin_slug = plugin_basename( ZOZOEMA_TOP_BAR_FILE );
	}
	...
	public function add_hooks()
    {
		...
		add_action( 'ema4wp_admin_enqueue_assets', array( $this, 'load_assets' ), 10, 2 );

		// for BC with ZozoEMA for WP < 3.0
		add_filter( 'ema4wp_menu_items', array( $this, 'add_menu_item' ) );
	}
	...
	public function init() {
		...
		// TODO: Use ema4wp capability here
		if( ! current_user_can( 'manage_options' ) ) {
			return false;
		}

		// register settings
		register_setting( 'zozoema_top_bar', 'zozoema_top_bar', array( $this, 'sanitize_settings' ) );
	...
	public function add_menu_item( array $items ) {
			...
			$item = array(
				'title' => __( 'ZozoEMA Top Bar', 'zozoema-top-bar' ),
				'text' => __( 'Top Bar', 'zozoema-top-bar' ),
				'slug' => 'top-bar',
	...
	public function add_plugin_settings_link( array $links ) {
		$settings_link = sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=zozoema-for-wp-top-bar' ), __( 'Settings', 'zozoema-for-wp' ) );
	...
	public function add_plugin_meta_links( array $links, $file ) {
		...
		$links[] = sprintf( __( 'An add-on for %s', 'zozoema-top-bar' ), '<a href="https://www.ema4wp.com/#utm_source=wp-plugin&utm_medium=zozoema-top-bar&utm_campaign=plugins-page">ZozoEMA for WordPress</a>' );
		return $links;
	}
	...
	public function load_assets( $suffix, $page = '' ) {
		...
		wp_enqueue_script( 'zozoema-top-bar-admin', $this->asset_url( "/js/admin{$suffix}.js" ), array( 'jquery', 'wp-color-picker' ), ZOZOEMA_TOP_BAR_VERSION, true );
		return;
	}
	...
	public function show_settings_page() {
		...
		require ZOZOEMA_TOP_BAR_DIR . '/views/settings-page.php';
	}
	...
	protected function name_attr( $option_name ) {
		return 'zozoema_top_bar[' . $option_name . ']';
	}
	...
	public function footer_text( $text ) {
		...
		if( ( isset( $_GET['page'] ) && strpos( $_GET['page'], 'zozoema-for-wp-top-bar' ) === 0 ) ) {
			$text = sprintf( 'If you enjoy using <strong>ZozoEMA Top Bar</strong>, please leave us a <a href="%s" target="_blank">★★★★★</a> rating. A <strong style="text-decoration: underline;">huge</strong> thank you in advance!', 'https://wordpress.org/support/view/plugin-reviews/zozoema-top-bar?rate=5#postform' );
		}
