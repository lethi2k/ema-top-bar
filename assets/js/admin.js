...
var $selectList = $(document.getElementById('select-zozoema-list'));
...
function maybeShowRequiredFieldsNotice() {
  ...
  $.get(ajaxurl + '?action=ema4wp_get_list_details&ids=' + listId).then(function (lists) {
    // iterate over selected lists
    for (var i = 0; i < lists.length; i++) {
      var list = lists[i]; // iterate over list fields

      for (var j = 0; j < list.merge_fields.length; j++) {
        var field = list.merge_fields[j]; // if field other than email_address is required, show notice and stop loop

        if (field.tag !== 'email_address' && field.required) {
...
$context.find('.ema4wp-color').wpColorPicker(); // if a list changes, check which fields are required
