...
var COOKIE_NAME = 'ematb_bar_hidden';
...
function Bar(wrapperEl, config) {
  var barEl = wrapperEl.querySelector('.ematb-bar');
  var iconEl = document.createElement('span');
  var formEl = barEl.querySelector('form');
  var responseEl = wrapperEl.querySelector('.ematb-response');
  var visible = false;
  ...
  function init() {
    // remove "no_js" field
    var noJsField = barEl.querySelector('input[name="_ematb_no_js"]');
    ...
    request.onreadystatechange = function () {
      ...
      if (this.status >= 200 && this.status < 400) {
        try {
          ...
          console.log('ZozoEMA Top Bar: failed to parse AJAX response.\n\nError: "' + error + '"');
          return;
        }
  ...
  function showResponseMessage(msg) {
    ...
    responseEl.className = 'ematb-response';
    var labelEl = document.createElement('label');
    labelEl.className = 'ematb-response-label';
  ...
  function calculateDimensions() {
    ...
    wrapperEl.className = wrapperEl.className.replace('ematb-icon-inside-bar', '');

    if (elementsWidth + iconEl.clientWidth + 200 < barEl.clientWidth) {
      wrapperEl.className += ' ematb-icon-inside-bar'; // since icon is now absolutely positioned, we need to set a min height
...
document.addEventListener('DOMContentLoaded', function () {
  var element = document.getElementById('zozoema-top-bar');
  window.ZozoEMATopBar = new Bar(element, window.ematb);
});
